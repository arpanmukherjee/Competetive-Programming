...
lld solve(int ind, int sum,int tight)
{
	if(dp[ind][sum][tight] == -1)
	{
		...
		else
		{
			...
			{
				fr(i,s[ind]-'0')
					ans += solve(ind+1, (sum+square(i))%3, 0);
				ans += solve(ind+1, (sum+square(i))%3, 1);
			}
			else
			{
				fr(i,10)
					ans += solve(ind+1, (sum+square(i))%3, tight);
			}
		}
		dp[ind][sum][tight] = ans;
	}
