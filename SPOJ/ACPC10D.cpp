...
lld smallest(lld x, lld y, lld z)
{
...
int main()
{
	...
	while(n!=0)
	{
		...
		a[0][2]+=a[0][1];
		a[1][0]+=a[0][1];
		a[1][1]+=smallest(a[1][0],a[0][1],a[0][2]);
		a[1][2]+=smallest(a[0][2],a[0][1],a[1][1]);
		fue(i,2,n-1)
		{
			a[i][0]+=min(a[i-1][0],a[i-1][1]);
			a[i][1]+=min(smallest(a[i-1][0],a[i-1][1],a[i-1][2]),a[i][0]);
			a[i][2]+=smallest(a[i][1],a[i-1][2],a[i-1][1]);
