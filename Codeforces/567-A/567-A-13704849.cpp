...
int main()
{
    ...
    fr(i,n)
        cin>>a[i];
    sp=' ';
    cout<<labs(a[0]-a[1])<<" "<<labs(a[0]-a[n-1])<<endl;
    fu(i,1,n-1)
        cout<<min(labs(a[i]-a[i-1]),labs(a[i]-a[i+1]))<<" "<<max(labs(a[i]-a[0]),labs(a[i]-a[n-1]))<<endl;
    cout<<labs(a[n-1]-a[n-2])<<" "<<labs(a[0]-a[n-1])<<endl;
    return 0;
}