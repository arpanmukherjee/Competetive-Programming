...
int main()
{
	...
	fr(i,n)
	{
		a.pb(mp(dist(r1,ar[i]),dist(r2,ar[i])));
	...
	fr(i,n)
	{
		...
		fr(j,n)
		{
			if(i==j)
				continue;
			if(a[j].ff>a[i].ff)
				temp=max(temp,a[j].ss);
		}
		ans=min(ans,a[i].ff+temp);
		//println_lld(ans);
	}
	temp=a[0].ss;
	fu(i,1,n)
		temp=max(temp,a[i].ss);
	println_lld(min(ans,temp));
	return 0;
}