...
void build(int node,int start,int end)
{
	if(start==end)
	{
		tree[node][0]=tree[node][1]=a[start];
		//cout<<"Node No: "<<node<<" and elements: "<<tree[node][0]<<" "<<tree[node][1]<<endl;
	}
	else
	{
		int mid=(start+end)>>1;
...
pii query(int node,int start,int end,int left,int right)
{
	...
	else
	{
		...
		p1.ff=max(p1.ff,p2.ff);
		p1.ss=min(p1.ss,p2.ss);
		return p1;
	}
}
