...
{
	double dis1=dist(x1,x2,y1,y2);
	if(dis1>double(r1+r2))
		return false;
	else if(dis1<=double(abs(r1-r2)))
		return true;
	else
		return false;
}

bool overlap(int x1,int y1,int x2,int y2,int r1,int r2)
{
	double dis1=dist(x1,x2,y1,y2);
	if(dis1>=(double)(r1+r2))
		return true;
...
int main()
{
	...
	while(q--)
	{
		...
		y=scan_d();
		r1=scan_d();
		if(inside(0,0,x,y,r,r1) and overlap(0,0,x,y,r-d,r1))
		{
			//cout<<x<<" "<<y<<" "<<r1<<endl;
			ans++;
		}
